...
object Graph {
  ...
  def getGroupFeeds(token: String, groupId: String, pageLimit: Int = 10, nextUrl: String = null, results: List[GroupFeed] = List()): List[GroupFeed] = {
    val resString: HttpResponse[String] = Http(s"https://graph.facebook.com/v2.11/$groupId/feed?fields=created_time,id,message,updated_time,caption,story,description,from,link,name,picture,status_type,type,shares,permalink_url,to,message_tags&limit=100&access_token=$token")
      .asString
    ...
    val newResults: List[GroupFeed] = results ::: data.validate[JsArray].getOrElse(Json.arr()).as[List[JsObject]]
      .map{ rawFeed => GroupFeed(
        ...
        (rawFeed \ "message_tags").validate[List[JsObject]].getOrElse(List()).map { obj => MessageTag(
