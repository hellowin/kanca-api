...
object Graph {
  ...
  private def getListResult(req: HttpRequest): FBListResult = {
  ...
  def getGroupFeeds(token: String, groupId: String, pageLimit: Int = DEFAULT_PAGE_LIMIT, nextUrl: String = null, results: List[GroupFeed] = List()): List[GroupFeed] = {
    val FBListResult(data, next) = getListResult(Http(s"$FB_URL/$groupId/feed")
      .params(Seq(
        "method" -> "GET",
    ...
    val newResults: List[GroupFeed] = results ::: data
      .map { rawFeed =>
        GroupFeed(
    ...
    getGroupFeeds(token, groupId, pageLimit - 1, next.get, newResults)
  }
